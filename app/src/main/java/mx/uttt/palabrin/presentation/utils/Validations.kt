...
class Validations @Inject constructor(
    ...
    fun validateWeight(weight: Double): ValidationResult {
        if (weight <= 0.0) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_weight_invalid)
            )
        }
        if (weight > 500) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_weight_too_high)
            )
        }
    ...
    fun validateHeight(height: Double): ValidationResult {
        if (height <= 0.0) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_height_invalid)
            )
        }
        if (height > 3.0) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_height_too_high)
            )
        }
        return ValidationResult(successful = true)
    }
    ...
    fun validateAge(age: Int): ValidationResult {
        if (age <= 0) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_age_invalid)
            )
        }
        if (age > 120) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_age_too_high)
            )
        }
        return ValidationResult(successful = true)
    }

    fun validateDiseases(diseases: List<String>): ValidationResult {
        if (diseases.any { it.isBlank() }) {
            return ValidationResult(
                successful = false,
                errorMessage = context.getString(R.string.val_diseases_invalid)
            )
        }
        return ValidationResult(successful = true)
    }
